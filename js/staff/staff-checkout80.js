...
async function savePaymentDetails(transactionId, cartItems, cashPaid, subtotal, salesTax, discount, pointDiscount, totalPrice, changes, memberDetails) {
    try {
        ...
        const paymentRef = doc(collection(db, 'orders'));
        const paymentData = {
    ...
    try {
        ...
        if (!isNaN(cashPaid) && change >= 0) {
            ...
            if (userId) {
                ...
                const memberDetails = membershipInfo.memberDetails;

                // Save payment details
                // const transactionDetails = await collectTransactionDetails();
                const transactionId = await generateTransactionID();
                await savePaymentDetails(
                    ...
                    totalPrice,
                    changes,
                    membershipInfo.valid ? memberDetails : null
                );
                ...
                if (membershipInfo.valid) {
                    ...
                    if (!isNaN(pointsToRedeem) && pointsToRedeem > 0) {
                        await redeemPoints(memberDetails.membershipId, pointsToRedeem);
                    }
                }
                ...
                await clearCart(userId); // Implement clearCart function to empty the user's cart

            } else {
                // No membership, just complete the payment without adding points
...
async function updateMemberPoints(membershipId, pointsToAdd) {
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            console.log("User not found with membershipId:", membershipId);
        }
    } catch (error) {
...
async function redeemPoints(membershipId, pointsToRedeem) {
    try {
        const usersCollection = collection(db, 'users');
        console.log("Executing query for membershipId:", membershipId);
        ...
        if (!querySnapshot.empty) {
            ...
            await updateDoc(userDoc.ref, { points: newdeductedPoints });
