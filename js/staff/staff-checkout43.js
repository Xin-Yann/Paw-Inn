...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function verifyMember(name) {
        try {
            ...
            const usersCollection = collection(db, 'users');
            const q = query(usersCollection, where('name', '==', trimmedName));
    ...
    async function checkMembership() {
        ...
        if (username) {
            ...
            if (querySnapshot.empty) {
                ...
                // const pointsToAdd = updatedMemberDetails.AddedPoints ?? 0; // Default to 0 if undefined
                // const pointsToRedeem = updatedMemberDetails.RedeemedPoints ?? 0;
                ...
                memberDetails = {
                    ...
                    points: newPoints
                    // AddedPoints: pointsToAdd || 0, // Ensure these variables are defined
    ...
    async function redeemPoints(membershipId) {
        try {
            const usersCollection = collection(db, 'users');
            const q = query(usersCollection, where('membershipId', '==', membershipId));
            const querySnapshot = await getDocs(q);
            ...
            if (!querySnapshot.empty) {
                ...
                const newDeductedPoints = 0;

                // Update user's points to zero
                const userRef = doc(db, 'users', userDoc.id);
                ...
                document.getElementById('member_points').textContent = `Points: ${currentPoints}`;
                ...
                if (memberInfoDiv) {
                    ...
                    pointsRedeemed.textContent = `Points Redeemed: ${currentPoints}`;
    ...
    document.getElementById('redeem-points').addEventListener('click', async () => {
        try {
            ...
            if (membershipInfo.valid) {
                ...
                // Parse points to redeem from the element's text content
                const pointsToRedeem = parseFloat(pointsToRedeemInput.textContent.replace('Points: ', ''));

                if (pointsToRedeem) {
                    await redeemPoints(membershipId);
                } else {
    ...
    function displayMessage(message, color) {
        const messageDiv = document.getElementById('messageDiv'); // Ensure there's a div with this ID in your HTML
    ...
    document.getElementById('cash-payment').addEventListener('click', async () => {
        try {
            ...
            let pointsToRedeem = 0; // Default value if element is not found

            if (pointsToRedeemElement) {
                const textContent = pointsToRedeemElement.textContent.replace('Points Redeemed: ', '');
                pointsToRedeem = parseInt(textContent); // Parsing as integer
            ...
            if (!isNaN(cashPaid) && change >= 0) {
                ...
                if (userId) {
                    ...
                    if (membershipInfo.valid) {
                        ...
                        // await checkMembership(updatedMemberDetails);
    ...
    async function savePaymentDetails(transactionId, cartItems, cashPaid, subtotal, salesTax, discount, pointDiscount, totalPrice, changes, memberDetails) {
        try {
            ...
            if (isEmailSent) {
                ...
                return; // Prevent sending multiple emails
            } else if (!isEmailSent) {
    ...
    document.getElementById('no').addEventListener('click', async (event) => {
        window.location.href = "../staff/staff-sales.html";
    });
