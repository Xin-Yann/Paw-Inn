...
async function fetchDataAndDisplay() {
    try {
        ...
        for (const { category, collectionName } of roomCategories) {
            ...
            const dogDocRef = doc(collection(db, 'rooms'), category);
            const dogRoomsCollectionRef = collection(dogDocRef, collectionName);
            ...
            for (const doc of dogRoomsQuerySnapshot.docs) {
                ...
                const roomHTML = `
                    <div class="rooms-container">
                        ...
                        <img class="card-img-top" src="/image/${category}/${roomData.room_image}" alt="${roomData.room_name}">
                        ...
                        <div class="card-body">
                            ...
                            <button class="btn btn-primary submit" data-room-name="${roomData.room_name}" data-category="${category}" product_price="${roomData.room_price}">Book Now</button>
                        </div>
                    </div>
                `;
        ...
        bookButtons.forEach(button => {
            button.addEventListener('click', (event) => {
                ...
                const roomCategory = button.getAttribute('data-category');
                ...
                // Store selected room name, category, and price in sessionStorage
                sessionStorage.setItem('selectedRoomName', roomName);
                sessionStorage.setItem('selectedRoomCategory', roomCategory);
